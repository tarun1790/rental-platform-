...
export const ScribbleMap: React.FC<ScribbleMapProps> = ({
  ...
  useEffect(() => {
    ...
    if (selectedListing && selectedListing.nearbyPointsOfInterest) {
      const homeLoc = selectedListing.propertyAddress?.location || (selectedListing.propertyAddress as any)?.coordinates;
      if (homeLoc && typeof homeLoc.latitude === 'number' && typeof homeLoc.longitude === 'number') {

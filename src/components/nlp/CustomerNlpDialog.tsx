                  ...
                  >
                    ...
                    {msg.matchedProperties && msg.matchedProperties.length > 0 && (
                      <div className="mt-4 space-y-3 pt-2">
                        ...
                        {(() => {
                          const isExpanded = expandedCardMsgId === msg.id;
                          const displayProperties = isExpanded ? msg.matchedProperties : msg.matchedProperties.slice(0, 6);

                          return (
                            <>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {displayProperties.map((rec) => (
                                  <div
                                    key={rec.listing.id}
                              ...
                              {msg.matchedProperties.length > 6 && (
                                <div className="flex justify-center pt-2">
                                  <button
                                    onClick={() => setExpandedCardMsgId(isExpanded ? null : msg.id)}
                                    className="px-4 py-1.5 rounded-xl text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 transition-all flex items-center gap-1.5 cursor-pointer shadow-sm"
                                  >
                                    <span>{isExpanded ? 'Show Top 6 Only' : `Show All ${msg.matchedProperties.length} Verified Homes`}</span>
